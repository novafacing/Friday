...
If $cmdline[0] <> 3 Then
	ConsoleWrite("Wrong number of args supplied: " & $cmdline[0] & @CRLF)
	ConsoleWrite(@CRLF)
...
If $processPID = -1 Then
	ConsoleWrite("Process ID not found, are you sure the process is running?")
	Exit
EndIf

Local $hProc = _NtOpenProcess($processPID)
If @error Then
	ConsoleWrite("Ntstatus: 0x" & Hex($hProc,8) & @CRLF)
	Exit
Else
	$hProc = $hProc
EndIf
...
Else
	ConsoleWrite("CreateProcessAsUserW: " & _WinAPI_GetLastErrorMessage() & @CRLF)
EndIf
...
Func _SetTokenInformation($hToken, $iTokenInformation, $vTokenInformation, $iTokenInformationLength)
	...
	If @error Or Not $aCall[0] Then
		ConsoleWrite("SetTokenInformation: " & _WinAPI_GetLastErrorMessage() & @CRLF)
